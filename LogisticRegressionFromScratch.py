...
class LogisticRegressionScratch(object):
    ...
    def predict_proba(self, X):
        ...
        Xw = np.dot(X, self.weights_array)  # X ⋅ W
        P = 1 / (1 + np.exp(-Xw))           # Inverse logit
    ...
    def compute_hessian(self, X, P):
        ...
        Q = P * (1 - P)                   # Q = P * (1 - P)
        XQ = np.dot(X.T, Q[:, np.newaxis] * X)  # Hessian: Xᵀ ⋅ Diag(Q) ⋅ X
        H = XQ
    ...
    def update_weights(self, X, y):
        ...
        self.prior_w = self.weights_array.copy()  # Store previous weights
    
        H_inv = np.linalg.inv(H)  # Inverse of Hessian
        update = np.dot(H_inv, G)  # H⁻¹ ⋅ G
        self.weights_array = self.weights_array - update  # w_t = w_{t-1} - H⁻¹ ⋅ G
...
# YOUR CODE HERE
# Step 1: Create an instance of your custom logistic regression class
...
# YOUR CODE HERE
# Step 1: Create a scikit-learn LogisticRegression model with high C (low regularization)
lr_sk = LogisticRegression(C=10**10, max_iter=1000)

# Step 2: Train the model on the same features and labels
lr_sk.fit(X, y)
